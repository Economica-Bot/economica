...
export default class implements Command {
	...
	public execute = new ExecutionBuilder().setExecution(async (ctx) => {
		...
		const botStats = `Websocket Ping: \`${ctx.client.ws.ping}ms\`\n`
			...
			+ `Commands Ran: \`${(
				await CommandEntity.find({
					relations: ['member', 'member.guild'],
					where: { member: { guildId: ctx.guildEntity.id } },
				})
			).length
			}\``;
		const memberStats = `Roles: \`${ctx.interaction.member.roles.cache.size}\`\n`
			+ `Commands Used: \`${(
				await CommandEntity.find({
					where: { member: { userId: ctx.memberEntity.userId, guildId: ctx.guildEntity.id } },
				})
			).length
			}\`\n`
