	...
	public execute = new ExecutionBuilder()
		...
		.setOptions([
			new ExecutionBuilder()
				...
				.setPagination(
					(ctx) => Transaction.find({
						relations: ['target', 'agent', 'guild'],
						where: { guild: { id: ctx.interaction.guildId } },
					}),
					(transaction) => new ExecutionBuilder()
						...
						.setExecution(async (ctx, interaction) => {
							...
							const row = new ActionRowBuilder<ButtonBuilder>().setComponents([
								new ButtonBuilder().setCustomId('transaction_delete').setLabel('Delete').setStyle(ButtonStyle.Danger),
							]);
				...
				.setExecution(async (ctx, interaction) => {
					...
					return new ExecutionBuilder()
						...
						.setPagination(
							(ctx) => Transaction.find({
								relations: ['target', 'agent', 'guild'],
								where: { guild: { id: ctx.interaction.guildId }, target: { userId: user.id } },
							}),
							(transaction) => new ExecutionBuilder()
								.setName(transaction.type)
								.setValue(transaction.id)
