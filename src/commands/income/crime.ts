...
export default class implements Command {
	...
	public execute = new ExecutionBuilder().setExecution(async (ctx) => {
		...
		if (Math.random() * 100 > chance) {
			await ctx
				.embedify('warn', 'user', `You were caught and fined ${ctx.guildEntity.currency}${parseNumber(fine)}.`)
				.send();
			...
			await ctx
				.embedify(
					'success',
					'user',
					`You successfully committed a crime and earned ${ctx.guildEntity.currency}${parseNumber(amount)}.`,
				)
				.send();
			await recordTransaction(
				ctx.client,
