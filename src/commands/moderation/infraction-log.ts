...
export default class implements Command {
	...
	public execute = async (ctx: Context): Promise<void> => {
		...
		if (subcommand === 'view') {
			const channelId = ctx.guildDocument.infractionLogId;
			if (channelId) {
			...
			if (!channel.permissionsFor(ctx.member).has('SEND_MESSAGES') || !channel.permissionsFor(ctx.member).has('EMBED_LINKS')) {
				...
				await ctx.guildDocument.updateOne({ infractionLogId: channel.id });
				await ctx.embedify('success', 'user', `Infraction log set to ${channel}.`, false);
			}
		} else if (subcommand === 'reset') {
			await ctx.guildDocument.update({ infractionLogId: null });
