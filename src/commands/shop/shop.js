...
module.exports = {
    ...
    options: [
      ...
      ***REMOVED*** {
            ...
            options: [
                {
                    name: 'confirm',
                    description: 'Are you sure you want to delete all shop items forever?',
    ...
    async run(interaction, guild, author, options, fops) {
        ...
        if (_subcommand === 'view') {
            ...
            if (items.length === 0) {
                embed.setDescription(`There are currently \`0\` items in the shop.\nAsk your Economy Manager to add some!`)
            } else {
                const currencySymbol = await util.getCurrencySymbol(guild.id)
                embed.setDescription(`There are currently \`${items.length}\` items in the shop. Use the \`item view\` command to view detailed item stats, use the \`item buy\` command to purchase an item!`)
                items.forEach(item => {
                    embed.addField(`${currencySymbol}${item.price > 0? util.num(item.price) : 'Free'} - ${util.cut(item.name)}`, util.cut(item.description || 'A very interesting item.', 200), item.description?.length > 100 ? false : true)
                })
            }
            ...
            if (fops.confirm) {
                ...
                embed.setDescription(`Successfully deleted all items in the ${guild.name} shop.`)
            }
        }
