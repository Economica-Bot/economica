		...
		.setExecution(async (ctx) => {
			...
			if (amount) {
				...
				await ctx
					...
					.addFields(items.map((item) => ({ name: `**${item.listing.name}** - ${ctx.guildEntity.currency} \`${parseNumber(item.listing.generatorAmount)}\``, value: `**Ready:** <t:${Math.trunc((item.lastGeneratedAt.getTime() + item.listing.generatorPeriod) / 1000)}:R>` } as APIEmbedField)))
					.send(true);
			}
		});
}
