	...
	public execution = new Router()
		.get('', (ctx) => new ExecutionNode()
			...
			.setOptions(...Object
				...
				.map(([name, module]) => ['select', `/${module}`, name, `\`${module.enabled ? 'ENABLED' : 'DISABLED'}\`\nType: \`${module.type}\``] as const)))
		.get('/:module', (ctx, params) => new ExecutionNode()
			.setName(params.module)
			.setDescription(`\`${ctx.guildEntity[params.module].enabled ? 'ENABLED' : 'DISABLED'}\`\nType: \`${ctx.guildEntity[params.module].type}\``)
			.setOptions(ctx.guildEntity.modules[params.module].enabled
				? ['button', `/${params.module}/disable`, 'Disable']
				: ['button', `/${params.module}/enable`, 'Enable']))
		.get('/:module/:action', async (ctx, params) => {
			...
			const newCommandData = ctx.interaction.client.commands.filter((command) => command.metadata.module === module).map((command) => command.metadata.toJSON());
