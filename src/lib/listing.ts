...
export const displayListing = (ctx: Context, listing: Listing) => {
	const listingEmbed = ctx
		.embedify(
			'info',
			'user',
			`**Id**: \`${listing.id}\` | **Created:** ${getFormattedCreateTimestamp(
				listing,
			)} | **Expires:** ${getFormattedExpiresTimestamp(listing)}`,
		)
		.setAuthor({
		...
		.addFields([
			{
				...
				value: `${ctx.guildEntity.currency} \`${parseNumber(listing.price)}\``,
				inline: true,
			},
			{
				name: `${Emojis.MONEY_BAG} Required Treasury`,
				value: `${ctx.guildEntity.currency} \`${parseNumber(listing.treasuryRequired)}\``,
				inline: true,
			},
			{
			...
			{ name: `${Emojis.TILES} In Stock`, value: `\`${listing.stock > 0}\``, inline: true },
			{
				...
				value: listing.rolesRequired.length ? listing.rolesRequired.map((role) => `<@&${role}>`).join('\n') : '`None`',
				inline: true,
			},
			{
				name: `${Emojis.RED_DOWN_ARROW} Roles Removed`,
				value: listing.rolesRemoved.length ? listing.rolesRemoved.map((role) => `<@&${role}>`).join('\n') : '`None`',
				inline: true,
			},
			{
	...
	if (listing.type === 'GENERATOR') {
		listingEmbed.addFields([
			{ name: 'Generator Period', value: `\`${ms(listing.generatorPeriod)}\``, inline: true },
			{
				...
				value: `${ctx.guildEntity.currency} \`${parseNumber(listing.generatorAmount)}\``,
				inline: true,
			},
		]);
	}
