...
/**
 * registers the command user's selection. Returns that selection or sends a time-out error message.
 * @param {Message} message - the parent message of the active channel (in which to listen)
 * @param {array} arr - the array of items in the selection (userIDs, items, etc...)
 * @param {number} time - expiration time for the collector
 * @param {string} type - [optional] what the elements of arr are. Creates a selection embed if possible. Options: 'user'
 */
module.exports.registerMemberSelection = (message, arr, time, type = def) => {
     if (type || !type === def) {
          if (type === 'user') {
               // show selection to command user
               this.memberSelectEmbed(message, arr)
          }
     }

     // register command user's selection
     const collector = this.createNumberCollector(message, arr.length, time)
     collector.on('collect', (m) => {})
     collector.on('end', (c) => {
          if (c.size === 0) return message.channel.send(`:hourglass: Time ran out (${(time / 1000).toFixed(0)})`)
          return c.content
     })
}
