const { cSymbol, prefix, income } = require('../config.json')
...
const incomeCache = {} // guildID-cmd: [min, max]
...
/**
 * update the min and max profit of an income command
 * @param {string} _id - the id of the guild
 * @param {string} cmd - the name of the income command
 * @param {number} min - the new minimum profit
 * @param {number} max - the new maximum profit
 */
module.exports.setIncome = async (_id, cmd, min, max) => {
     ...
     return await mongo().then(async (mongoose) => {
          try {
             ...
             ***REMOVED*** {
               ...
               incomeCache[`${_id}-${cmd}`] = [result[command].min, result[command].max]
...
/**
 * get the income min and max of an income command in an IntArray: [min, max]
 * @param {string} _id - the id of the guild
 * @param {string} cmd - the name of the income command
 */
module.exports.getIncome = async (_id, cmd) => {
     const command = cmd.toString()
     const cached = incomeCache[`${_id}-${cmd}`]
     if (cached) return cached
     ...
     return await mongo().then(async (mongoose) => {
          try {
               ...
               let income = [config.income[command].min, config.income[command].max]
               if (result[command].min && result[command].max) {
                    income = [result[command].min, result[command].max]
               }
