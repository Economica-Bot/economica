...
export class Layer {
	...
	handle: (ctx: Context, params: unknown) => Awaitable<ExecutionNode | string>;

	constructor(path: string, handle: (ctx: Context, params: unknown) => Awaitable<ExecutionNode | string>) {
		this.path = path;
		this.handle = handle;
	}
}
